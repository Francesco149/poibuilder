...
- RETRO EXPORT PIPELINE & SONY PSP HOMEBREW RENDERER (`PBMapExporter`, `pbm_conv.py`, `retro_engine/psp/`):
  ...
  - Complete PSP homebrew application (`retro_engine/psp/`):
    ...
    - Texture swizzling & fillrate optimization: swizzled 16-bit textures in RAM into 16x8 byte tiles on load and enabled `sceGuTexMode(psm, 0, 0, 1)`, eliminating texture cache thrashing and memory bus congestion when getting close to large textures (e.g. the 512x512 cylinder).
    - CPU Frustum Culling & 16-bit Framebuffer (`GU_PSM_5551`): extracted Gribb-Hartmann view-projection frustum planes and implemented fast AABB testing in `is_box_in_frustum()`, culling ~70+ off-screen meshes and reducing GE triangle submission from 3840 down to ~140. Switched framebuffer to 16-bit `GU_PSM_5551`, halving eDRAM write bandwidth and doubling pixel fillrate. Enabled `sceGuEnable(GU_CLIP_PLANES)` for hardware edge clipping.
    ...
    - Pitch controls: Triangle tilts camera UP (`cam_pitch += 1.8f * dt`), Square tilts camera DOWN (`cam_pitch -= 1.8f * dt`).
