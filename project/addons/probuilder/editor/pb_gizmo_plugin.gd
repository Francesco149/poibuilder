...
const VERTEX_COLOR := Color(0.65, 0.65, 0.65, 1.0)
const VERTEX_DOT_SIZE: float = 4.0
const VERTEX_DOT_SELECTED_SIZE: float = 8.0
...
func _redraw(gizmo) -> void:
	...
	# Wireframe (depth-tested, like ProBuilder's shape wireframe).
...
func _draw_selected_edges(gizmo, mesh_data: PBMeshData) -> void:
	...
	if selected_any and lines.size() >= 2:
		gizmo.add_lines(lines, get_material("pb_selected_edge", gizmo))
