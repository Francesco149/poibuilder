...
## The shared emitter reconstruction (holder scan, particle derivation, the
## billboard mesh builder) — kept in emitter_preview.gd so the viewer, the
## frame-pacing bench and the fly bench all draw the SAME preview.
...
## The reconstruction itself (stateless closed-form particle stream, the C
## reference in retro_engine/psp/psp_render.c, semantics in
## SPEC_RETRO_FORMAT.md §8) lives in emitter_preview.gd, shared with the
## bench so every consumer draws the SAME preview. The viewer keeps only its
## own clock (with the scroll-freeze) and its play-mode camera choice.

## Finds the emitters in an imported map and hides their texture carriers.
func _setup_emitters(root: Node) -> void:
	if root == null:
		return
	for holder in EmitterPreviewT.find_holders(root):
		emitter_previews.append(EmitterPreviewT.make_preview(holder, map_container))
...
func _update_emitters(delta: float) -> void:
	...
	if cam == null:
		return
	EmitterPreviewT.tick(emitter_previews, _emit_time, cam)
