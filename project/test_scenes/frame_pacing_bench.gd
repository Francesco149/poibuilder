...
## lights cannot carry them) — lives in bench_variants.gd (BenchVariants),
## shared verbatim with the interactive fly bench (fly_bench.gd).
##
...
func _run() -> void:
	...
	Engine.max_fps = 0
	# The report must name its machine: get_video_adapter_name() is how the
	# docs' tables (RTX 5060 vs the UHD 630 iGPU) stay pinned to real GPUs.
	var gpu := RenderingServer.get_video_adapter_name()
	print("[bench] gpu=%s driver=%s" % [gpu, RenderingServer.get_video_adapter_type()])

	if opts["ablate"] != "":
		await _run_ablation(opts)
		return

	var map := BenchVariants.load_variant(opts["variant"])
	if map == null:
		quit(1)
		return
	root.add_child(map)
...
func _run_ablation(opts: Dictionary) -> void:
	...
	for ablation in ["base"] + ablations:
		var map := BenchVariants.load_variant("pb")
		if map == null:
			quit(1)
			return
		root.add_child(map)
	...
	_print_profile(rows)
	# --out (what the launchers pass, pb_profile.<renderer>.json) must win:
	# the hardcoded default made the GL and Vulkan profiles clobber one file.
	_write_report({"variant": "pb", "profile": "ablation",
		"gpu": RenderingServer.get_video_adapter_name(),
		"renderer": RenderingServer.get_current_rendering_method(), "rows": rows},
		opts["out"] if opts["out"] != "" else "res://exports/bench/pb_profile.json")
	quit(0)
...
func _apply_ablation(map: Node3D, ablation: String) -> String:
	match ablation:
		...
		"no_shadows":
			var n := [0]
			BenchVariants.apply_to_lights(map, func(l: Light3D) -> void:
				if l.shadow_enabled:
...
func _print_profile(rows: Array) -> void:
	...
	for row: Dictionary in rows:
		...
		print("[profile]   without %-12s %6.2f ms  (%+.2f vs base)" % [
			row["variant"], row["median_ms"], row["delta_ms_vs_base"]])
...
func _summarize(variant: String, frames: PackedFloat64Array, map: Node3D,
	...
	BenchVariants.census(map, census)
	return {
		"variant": variant,
		"pass": pass_name,
		"frames": n,
