...
func _ready() -> void:
	...
	print(" 17. EDGE mode has a big hitbox: border edges pick from either side, and an")
	print("     edge seen through a face is grabbable (its highlight draws exactly on it).")
	print("     When both a near and far edge overlap, the visible one wins. Faces stay")
	print("     quads (n-gons): no triangulation diagonals appear in the edge list.")
	print("     The ELEMENT-space gizmo for an edge/vertex orients by the face normal")
	print("     of the side you selected it from (X cycles Element/Object/World).")
	print("")
