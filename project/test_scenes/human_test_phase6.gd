...
func _ready() -> void:
	...
	print(" 17. EDGE mode: wireframe renders as bold black strokes; the selected edge")
	print("     glows cyan on top. Verts in VERTEX mode are bold dark dots (cyan when")
	print("     selected). Faces stay quads (n-gons): no triangulation diagonals.")
	print(" 18. EDGE mode has a big hitbox: border edges pick from either side, and an")
	print("     edge seen through a face is grabbable (its highlight draws exactly on it).")
	print("     When both a near and far edge overlap, the visible one wins.")
	print(" 19. Clicking an element NEAR/UNDER the transform gizmo selects that element")
	print("     (it must not start dragging the previously selected element). Grabbing")
	print("     the gizmo arrows/rings themselves still works, and the ELEMENT-space")
	print("     gizmo orients by the face normal of the side you selected from (X cycles).")
	print("")
