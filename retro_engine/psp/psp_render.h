...
typedef struct {
     ...
     * splat/stamp tiles): `detail_const` >= 0 pins them to that ONE mip level
     * (the default), `detail_const` < 0 falls back to `detail_bias` added to
     * tex_lod_bias. Which meshes match is psp_render_detail_match(). */
    float detail_bias;
    int   detail_const;
