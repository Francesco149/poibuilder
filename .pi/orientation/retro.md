# Retro Pipeline — Pointers

The retro pipeline (Godot → `.glb`/`.pbm` → PSP / Godot viewers) has its own
deep docs. Read the relevant one; do not duplicate them here.

`PBMapExporter` is the ONLY `.pbm` writer: the GLB->PBM converters
(`retro_engine/pbm_conv.py`, `export/pb_pbm_converter.gd`) and the raylib demo
engine were retired — they existed to prove the format and had fallen behind.

| Topic | Doc |
|---|---|
| Authoring levels, export dialog knobs, what the export bakes, pitfalls | `retro_engine/RETRO-AUTHORING.md` |
| Real-PSP measurement: PSPLink over USB, the `./run_psp_hw.sh` loop, the hard-reset rule, why PPSSPP/desktop numbers are not evidence | `retro_engine/psp/HARDWARE-TESTING.md` |
| The engine's optimization inventory with measured cost of every decision (fill, texture-cache cliff, mip/LOD policy, per-frame budget) | `retro_engine/psp/OPTIMIZATION.md` |
| The `.pbm` byte format + consumer performance rules | `SPEC_RETRO_FORMAT.md` |
| The `.pbm` writer's format contract | `project/tests/test_pb_pbm_export.gd` (builds its own fixture, asserts the byte layout) |

Non-negotiables (details in the docs above):
1. A performance number is real only if it came from a PSP over USB
   (`./run_psp_hw.sh`). Emulators and desktop viewers are correctness-only.
2. Changed a measured default (LOD bias, filter, texture format, emitter
   budget)? Re-measure and update the row in the same commit.
3. Map-format changes: `./run_tests.sh` (the writer's format test) plus a
   PSP check. There is no second engine to re-test any more.
4. Build artifacts (`EBOOT.PBP`, `*.prx`, `PoiRetro_PSP.zip`, exports) are
   gitignored and regenerated by `./run_psp.sh` / `./run_tests.sh` /
   `./showcase_map.sh` — never commit them.

More non-negotiables (moved from CLAUDE.md, still current):
5. **Do not build on unmeasured performance assumptions** — not in code
   comments, not in docs, not in a summary. "This should be faster" is not a
   result; either measure it or state plainly that it is unmeasured.
6. **Never assume the PSP's presence — probe it**: `./psp_probe.sh` answers
   in ~5 s (USB device → udev symlink → PSPLink link, with a verdict at each
   layer). A session once shipped "no PSP connected" caveats without running
   any probe while the device sat answering on USB. If the probe says ABSENT,
   say so and ask for `./setup_psplink.sh` (30 seconds, once per device)
   before the session ends; mark perf-related work unverified rather than done.
7. Every optimization claim in the docs carries the device row that produced
   it. PPSSPP rasterises on the host GPU with a huge texture cache and no
   shared memory bus — it happily reports 60 fps for a build that spends
   27 ms of a 16.6 ms budget on the device. Never cite it for speed, and
   never conclude from it that a slowdown is fixed.

8. **Imported props have one texture plan, not a second export path.** Any plain
   `MeshInstance3D` (a `.glb` dragged in from an asset pack) is exported by
   `_export_plain_mesh` and its textures are sanitized once per export run by
   `pb_map_exporter.plan_imported_textures`: the sampled atlas region is cropped
   to the nearest power-of-two rect (UVs remapped by `(uv - origin) * scale`),
   alpha is taken from the pixels rather than the material, identical textures
   are shared by content, and the node's WORLD transform is used (the export
   tree is flat; the placement of a glTF import lives on its wrapper node).
   Both the GLB route and the direct `.pbm` writer read the same plan — read it
   before touching either. `retro_engine/RETRO-AUTHORING.md` §7 is the
   authoring-side summary; the device rows are in HARDWARE-TESTING.md.
